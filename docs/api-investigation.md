<!--
  This file is a verbatim copy of `openCsiTool_API_Investigation_Report.md`
  at the repository root, kept here so the docs tree is self-contained.

  It is the PRIMARY EVIDENCE for everything this tool does. It has deliberately
  NOT been edited to match the implementation: where the implementation and this
  report disagree, that disagreement is a finding, not something to paper over.
  The implementation's own record of what it depends on is
  `opencsi contract-check` and `tests/test_mapping_regression.py`.
-->
