...
391 offline tests and a 42-check replay of the original investigation baselines.
...
| Verifiable offline | **Yes** | 391 tests, no network, run under `unittest` and under `pytest`. Also clean under `-W error::ResourceWarning`. |
...
**391 tests, all passing, fully offline**, run with `unittest`:

```
$ cd tests && python -m unittest discover -s . -p "test_*.py" -t .
Ran 391 tests in 11.1s
OK
```
...
2. Those exact responses are committed as sanitized fixtures and replayed by 391
   offline tests.
...
Every new test was confirmed to fail against the reintroduced bug (`12 != 10`,
`2 != 130`) before the fixes were restored, so they guard the cause rather than
documenting whatever the code currently does.

Taking that lesson seriously meant grepping for *every* literal exit code rather
than re-reading only the two commands that had been wrong. That found a fifth
instance: `login --manual` caught `KeyboardInterrupt` in the same branch as
`EOFError` and returned the usage code — so Ctrl+C at the hidden cookie prompt
exited 2 ("invalid arguments") while Ctrl+C anywhere else exited 130, for the
identical action on the identical key. Interrupts are now a named, documented
constant used at all three sites that report one, and each branch is tested.
...
```
$ python -m unittest discover -s . -p "test_*.py" -t .   # 391 tests, OK
$ pytest                                                  # 391 passed
```
...
```bash
cd tests && python -m unittest discover -s . -p "test_*.py" -t .   # 391 tests
