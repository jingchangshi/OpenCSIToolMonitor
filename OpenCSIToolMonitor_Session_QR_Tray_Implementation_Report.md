...
| 测试 | **完成** | **729 项测试**（728 通过、1 跳过、119 个 subtest），`pytest` 与 `unittest` 双跑全绿 |
| Windows 实机验证 | **完成** | 实测 CLI、扫码、托盘、冻结二进制、入口点 |
| 文档 | **完成** | 5 份文档 + README + 本报告 |
| 规范提交 | **完成** | 33 个修改源码/测试的提交（§11 完整列出）；其中 11 个修复了通过**运行真实产物**才发现的缺陷 |
...
后续在真机运行中又发现并修复了八个（缺陷 4–11，见 §10）。其中缺陷 8 与本节的三个不同：
...
**729 项测试：728 通过，1 跳过，119 个 subtest 通过。**

```
$ pytest
728 passed, 1 skipped, 119 subtests passed in 39.75s

$ python -m unittest discover -s tests -q
Ran 729 tests in 36.597s
OK (skipped=1)
```
...
缺陷 8 是"测试只覆盖了状态映射，没覆盖该状态下**动作能否达成目的**"；缺陷 10 与 11 是
"测试只覆盖了错误**分类**，没覆盖该分类是否**描述事实**，以及是否覆盖了**所有**能产生
该错误的路径"。

五者其实是同一件事：**测试断言的是代码写了什么，而不是用户能否得到他要的东西。**
...
缺陷 11 额外说明了一件事：**修好一个缺陷之后要问"还有哪里会这样"，而不是"测试绿了吗"。**
缺陷 10 的修复让所有测试通过，也让实测通过；如果就此收工，反应式路径会继续误报，而且只
在 401 恰好撞上授权页时才出现——一个比原缺陷更难复现的形态。
...
自 `cf34c1b` 起，**修改了源码或测试的提交共 33 个**，下表完整列出（最旧在前，覆盖
`cf34c1b` 到 `27528e8`）。全部以 `opencsi contributors <contributors@opencsi.invalid>`
署名。

表中没有、也不可能有的是**纯文档提交**：它们撰写、修订本报告，修正本报告对自身 SHA 的
引用，并把报告改写为中文。一个提交无法列出自己的 SHA，所以它们不可能出现在表里；
`27528e8` —— 最后一个修改源码或测试的提交 —— 是 §2 中命名的锚点。

这里刻意**不写"总提交数"**：那个数字每写一次文档提交就会失效，而写它的正是文档提交
本身。33 则是稳定的——文档提交不碰 `src/`、`tests/`、`packaging/`，所以这个数字不会
被本节自身的修订改变。判定标准同样是机械的：

```bash
git log --oneline cf34c1b~1..HEAD -- src tests packaging   # 33 行，即下表
```
