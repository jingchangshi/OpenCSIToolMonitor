...
**会话大约持续一小时。** 这是站点的性质，不是这里的偷懒——不存在 refresh token。
只要专用浏览器配置里的 **GitCode 登录态**还在，工具会在 Cookie 快到期时**静默重走一次
OAuth** 换回新的，通常你完全察觉不到。只有 GitCode 自己的登录态失效时，才需要重新登录。

**注意"重新读取 Cookie"救不活过期的会话**：浏览器里那份同时也过期了。续期必须重走
OAuth，这正是 `opencsi login --renew` 存在的原因。
...
**关于"已有托盘在运行"（退出码 2）**：窗口化版没有控制台，因此这一情形会弹出一个说明
恢复步骤的对话框，而不是一声不响地退出（见 §10 缺陷 19）。注意 PyInstaller 的 onefile
构建把真正的托盘作为**子进程**运行，所以用任务管理器"结束任务"或 `Stop-Process -Id`
只杀掉引导器，真正的托盘会作为孤儿进程继续持有单实例锁。此时去**通知区域**右键那个
图标选「退出」即可，而不必去结束进程。
